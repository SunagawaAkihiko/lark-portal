...
//
// ルーティング:
//   GET /staff    → 会社Wi-Fiチェック後に index.html を返す（スタッフ用入口・制限あり）
//   GET /staff/   → /staff へリダイレクト
//   各ツールページ(.html) → 制限なし（/staffの入口だけ制限すれば十分という方針）
//
// Wi-Fi制限の仕組み:
...
// ---- 静的ファイル配信 ----
// 各ツールページ(.html)はWi-Fi制限の対象外（/staffの入口だけで制限する方針）。
